...
{
    std::string format(
        const wnd::utils::ProcessTree::Process &process,
        std::string_view format)
    {
        return fmt::format(
            format, fmt::arg("app", process.name), fmt::arg("pid", process.pid),
            fmt::arg("ppid", process.ppid),
            fmt::arg(
                "vmRss", fmt::format("{:.2f}", process.memory.vmRss / b_to_m)),
            fmt::arg(
                "vmSize",
                fmt::format("{:.2f}", process.memory.vmSize / b_to_m)),
    ...
    void show(
        const wnd::utils::ProcessTree::Process &process, int depth,
        std::string_view format, std::string &out)
    {
