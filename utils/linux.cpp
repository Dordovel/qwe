...
namespace
{
    ...
    class Processes
    {
        private:
            ...
            wnd::utils::ProcessTree::Process convert(const Process& process)
            {
                ...
                adapter.p_cpu = std::round((100 * 
                                        (static_cast<float>(process.cpu - process.cpuLast)
                                            / (wnd::system::Cpu::get_cpu_total() - this->cpuLast))) * 10) / 10;
    ...
    public:
        wnd::utils::ProcessTree::Process get_tree_for_process(std::string_view pid)
        {
            ...
            auto iter = std::find_if(processes.begin(), processes.end(), [&pid](const auto& process){return process.pid == pid;});
            ...
            this->find_childs_for_process(adapter.pid, processes, std::move(this->_processes), adapter.child);
