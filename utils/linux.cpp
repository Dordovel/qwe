...
namespace
{
    ...
    class Processes
    {
        private:
            ...
            parse_process(std::string_view line)
            {
                auto pos = line.find(":");
            ...
            Processes::Process read_process(std::string_view pid)
            {
                ...
                std::ifstream stream(
                    root + std::string(pid) + std::string("/status"));
            ...
            wnd::utils::ProcessTree::Process convert(const Process &process)
            {
                ...
                float p_cpu =
                    static_cast<float>(process.cpu - process.cpuLast) /
                ...
                std::vector<wnd::utils::ProcessTree::Process> &ref)
            {
                for (auto &process : processes)
                {
                    if (process.ppid == pid)
                    {
                        ...
                        find_childs_for_process(
                            process.pid, processes, std::move(oldProcesses),
                            child);
                        process.memory =
                            wnd::ProcessMemory::get_memory_for_process(
                                process.pid);
                        process.cpu =
                            wnd::ProcessCpu::get_cpu_for_process(process.pid);

                        auto oldState = std::find_if(
                            oldProcesses.cbegin(), oldProcesses.cend(),
                            [&process](const auto &old)
            ...
            std::vector<Processes::Process> get_all_processes()
            {
                ...
                for (const auto &pathIterator :
                     std::filesystem::directory_iterator(p))
                {
                    if (pathIterator.is_symlink())
                        continue;
        ...
        public:
            wnd::utils::ProcessTree::Process
            get_tree_for_process(std::string_view pid)
            {
                ...
                auto oldState = std::find_if(
                    this->_processes.cbegin(), this->_processes.cend(),
                ...
                this->find_childs_for_process(
                    adapter.pid, processes, std::move(this->_processes),
                    adapter.child);
