...
#if (defined(__FreeBSD__) && defined(HAVE_LIBKVM))
#include <fcntl.h>
#include <kvm.h>
#include <sys/param.h>
...
namespace
{
    ...
    wnd::utils::ProcessTree::Process find_process(int pid)
    {
        ...
        int cnt;
        kinfo_proc *kp = kvm_getprocs(kd, KERN_PROC_PID, pid, &cnt);
    ...
    void find_childs_for_process(
        int pid, kinfo_proc *kinfo, int cnt,
        std::vector<wnd::utils::ProcessTree::Process> &ref)
    {
        for (int i = 0; i < cnt; ++i)
        {
            ...
            if (kp.ki_ppid == pid)
            {
                if (kp.ki_ppid == kp.ki_pid)
                    continue;
                wnd::utils::ProcessTree::Process info =
                    create_process_struct(&kp);
    ...
    void find_childs_for_process(
        int pid, std::vector<wnd::utils::ProcessTree::Process> &ref)
    {
